...
import { FaSearch, FaUser, FaRegHeart, FaThumbsUp, FaHeart, FaStar, FaPlane, FaHotel, FaMapMarkerAlt, FaCalendarAlt, FaArrowLeft } from 'react-icons/fa';
...
const HotelCard = ({ name, location, price, imageSrc }) => (
    <div className="flex bg-white shadow-lg rounded-xl p-6 mb-8 hover:shadow-2xl hover:scale-105 transition-all duration-500">
      ...
      <div className="w-1/3">
        <img src={imageSrc} alt={name} className="rounded-xl w-full h-full object-cover" />
      </div>
      <div className="w-2/3 pl-6 flex flex-col justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-2">Show Cooking Restaurant, Kids Club, Recreation Room</p>
          <p className="text-sm text-gray-500">2+ Guests</p>
        </div>
        <div className="flex justify-between items-center mt-6">
          <FaThumbsUp className="text-black text-xl" />
          <div className="flex flex-col items-end">
            <p className="text-[#023E8A] font-poppins font-bold text-lg mb-2">Rp. {price}</p>
            <button className="bg-[#023E8A] text-white rounded-lg px-14 py-1 hover:bg-[#969da5] transition-colors font-poppins font-bold">
              Detail
            </button>
          </div>
        </div>
    ...
    <div className="w-1/3 mr-8 -ml-60 bg-white p-4 rounded-lg shadow-md border border-black max-h-[1500px]">
      ...
      <div className="mb-6">
        <div className="flex flex-wrap gap-2">
          ...
          <button className="bg-gray-200 text-gray-700 rounded px-3 py-1">Homestay</button>
          <button className="bg-green-100 text-green-700 rounded px-3 py-1">Breakfast available</button>
        </div>
      </div>
      ...
      <div className="mb-4 border-t border-black pt-4">
        ...
        <div className="flex flex-col gap-1 text-sm">
          <label><input type="checkbox" className="mr-2" /> Promo for you</label>
          <label><input type="checkbox" className="mr-2" /> EPIC SALE</label>
          <label><input type="checkbox" className="mr-2" /> Best Price</label>
        </div>
      </div>
      ...
      <div className="mb-4 border-t border-black pt-4">
        ...
        <div className="flex flex-col gap-1 text-sm">
          {[1,2,3,4,5].map(star => (
            <label key={star}><input type="checkbox" className="mr-2" /> {Array(star).fill('⭐').join(' ')}</label>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6 border-t border-black pt-4">
        ...
        <div className="flex flex-col gap-1 text-sm">
          {["Villa", "Hotel", "Apartement", "Homestay"].map((type, index) => (
            <label key={index}><input type="checkbox" className="mr-2" /> {type}</label>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6 border-t border-black pt-4">
        ...
        <div className="flex flex-col gap-1 text-sm">
          {["Breakfast Available", "Early Check-in", "Airport Transfer", "Late Check-out", "Pools"].map((facility, index) => (
            <label key={index}><input type="checkbox" className="mr-2" /> {facility}</label>
          ))}
        </div>
      ...
      <div className="mt-6 border-t border-black pt-4">
        ...
        <div className="flex flex-col gap-1 text-sm">
          <label><input type="checkbox" className="mr-2" /> Bayar di hotel</label>
        </div>
      </div>
      ...
      <div className="mt-6 border-t border-black pt-4">
        ...
        <div className="flex flex-col gap-1 text-sm">
          {["Golf", "Indoor Sports", "Entertainment Area"].map((facility, index) => (
            <label key={index}><input type="checkbox" className="mr-2" /> {facility}</label>
          ))}
        </div>
        <div className="text-blue-500 text-sm mt-1 cursor-pointer">See All</div>
...
const HotelSearchPage = () => {
    return (
        <div className="bg-[#F8F8F8] w-full min-h-screen px-0 -pt-4 pb-10 flex flex-col items-center">
          <NavbarProfile />
        <div className="flex bg-white rounded-t-xl overflow-hidden shadow ml-[-756px] -mt-20">
