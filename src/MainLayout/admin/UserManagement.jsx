          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-4">
            {users.length > 0 ? (
              users.map((user) => (
                <div
                  key={user.userId}
                  className="bg-sky-50 border border-blue-100 rounded-xl p-6 shadow hover:shadow-xl hover:bg-sky-100 transition-all"
                >
                  <h3 className="text-xl font-bold text-blue-900">
                    {user.name}
                  </h3>
                  <p className="text-slate-500">{user.email}</p>
                  <span className="mt-2 inline-block px-3 py-1 bg-gradient-to-r from-sky-100 to-blue-100 text-blue-700 rounded-full text-xs font-medium shadow-sm">
                    {user.role}
                  </span>
